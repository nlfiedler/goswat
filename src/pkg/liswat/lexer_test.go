...
func TestLexerSimple(t *testing.T) {
	input := "(set foo bar)"
	expected := make([]expectedResult, 0)
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "set"})
	expected = append(expected, expectedResult{tokenSymbol, "foo"})
	expected = append(expected, expectedResult{tokenSymbol, "bar"})
	expected = append(expected, expectedResult{tokenCloseParen, ")"})
	expected = append(expected, expectedResult{tokenEOF, ""})
	verifyLexerResults(t, input, expected)
}
...
`
	...
	// 0
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "define"})
	expected = append(expected, expectedResult{tokenSymbol, "fact"})
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "lambda"})
	// 5
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "n"})
	expected = append(expected, expectedResult{tokenCloseParen, ")"})
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "if"})
	// 10
	expected = append(expected, expectedResult{tokenOpenParen, "("})
	expected = append(expected, expectedResult{tokenSymbol, "<="})
	expected = append(expected, expectedResult{tokenSymbol, "n"})
